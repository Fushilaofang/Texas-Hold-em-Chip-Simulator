...
private const val HEARTBEAT_INTERVAL_MS = 2_000L   // 每 2 秒发一次 ping
private const val HEARTBEAT_TIMEOUT_MS = 4_000L    // 4 秒无 pong 才视为掉线
        ...
        scope.launch {
            try {
                ...
                while (isActive) {
                    ...
                    // 同样地为服务端每个客户端连接设置 6 秒超时
                    runCatching { socket.soTimeout = 6000 }
                    launch {
                        handleClient(
                            socket = socket,
